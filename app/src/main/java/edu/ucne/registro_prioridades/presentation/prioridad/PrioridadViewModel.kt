...
class PrioridadViewModel @Inject constructor(
    ...
    private suspend fun existsWithDescription(descripcion: String): Boolean {
        return prioridadRepository.findByDescripcion(descripcion) != null
    }
        ...
        viewModelScope.launch {
            ...
            when {
                ...
                state.diasCompromiso <= 0 -> {
                    ...
                    try {
                        val exists = prioridadRepository.findByDescripcion(state.descripcion)
                        if (exists != null && exists.prioridadId != state.prioridadId) {
                            _uiState.update {
                                it.copy(errorMessages = "Ya existe una prioridad con esta descripción", successMessage = null)
                            }
                        } else {
    ...
    fun delete() {
        viewModelScope.launch {
            try {
                ...
                prioridadRepository.delete(entityToDelete)
                ...
                Log.e("PrioridadViewModel", "Error al eliminar la prioridad", e)

                _uiState.update {
                    it.copy(
                        errorMessages = "Error al eliminar la prioridad",
                        successMessage = null
                    )
                }
            }
