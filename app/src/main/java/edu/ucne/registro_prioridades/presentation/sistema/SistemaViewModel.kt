            ...
            SistemaUiEvent.Save -> {
                viewModelScope.launch {
                    val sistemaBuscado = sistemaRepository.getAll()
                        .find { sistema -> sistema.nombre.lowercase() == _uiState.value.nombre?.lowercase() }
                    ...
                    if (_uiState.value.errorNombre == "") {
                        try {
                            ...
                            Log.e("ViewModel", "Error al guardar el sistema: ${e.message}", e)
                            _uiState.update {
                                it.copy(errorNombre = "Error al guardar el sistema")
                            }
                        }
                    }
                }
